...
#get input arguments from JSON file
...
def random_classes(num_classes=12):
    ...
    return cls1000[:num_classes]

# Extract arguments with defaults
class Args:
    def __init__(self, config_dict):
        self.song = config_dict.get('song')
        self.resolution = config_dict.get('resolution', '512')
        self.duration = config_dict.get('duration')
        self.pitch_sensitivity = config_dict.get('pitch_sensitivity', 220)
        self.tempo_sensitivity = config_dict.get('tempo_sensitivity', 0.25)
        self.depth = config_dict.get('depth', 1)
        self.num_classes = config_dict.get('num_classes', 12)
        self.classes = config_dict.get('classes', random_classes(self.num_classes))
        self.sort_classes_by_power = config_dict.get('sort_classes_by_power', 0)
        self.jitter = config_dict.get('jitter', 0.5)
        self.frame_length = config_dict.get('frame_length', 512)
        self.truncation = config_dict.get('truncation', 1)
        self.smooth_factor = config_dict.get('smooth_factor', 20)
        self.batch_size = config_dict.get('batch_size', 30)
        self.use_previous_classes = config_dict.get('use_previous_classes', 0)
        self.use_previous_vectors = config_dict.get('use_previous_vectors', 0)
        self.output_file = config_dict.get('output_file', 'output.mp4')

args = Args(config)

if not args.song:
    raise ValueError("you must provide 'song' in the configuration file")
